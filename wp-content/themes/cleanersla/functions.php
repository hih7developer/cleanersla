...
function twentytwenty_sidebar_registration()
{
	...
	register_sidebar(
		array(
			'name'        => __('Footer Newsletter', 'twentytwenty'),
			'id'          => 'sidebar-2',
	...
	register_sidebar(
		array(
			'name'        => __('Header Logo', 'twentytwenty'),
			'id'          => 'header-logo',
			'before_widget' => '',
			'after_widget' => '',
			'description' => __('Widgets in this area will be displayed logo in the header.', 'twentytwenty'),
		)
	);
	...
	register_sidebar(
		array(
			'name'        => __('Footer Logo', 'twentytwenty'),
			'id'          => 'footer-logo',
			'before_widget' => '',
			'after_widget' => '',
			'description' => __('Widgets in this area will be displayed logo in the footer.', 'twentytwenty'),
		)
	);
}
...
function service_post_type()
{
	...
	$args = array(
		...
		'description'         => __('Service details', 'twentytwenty'),
		'labels'              => $labels,
...
// service home loop shortcode

function service_home_loop_shortcode()
{
	ob_start();
	get_template_part('template-parts/service', 'home_loop');
	return ob_get_clean();
}
add_shortcode('service_home_loop', 'service_home_loop_shortcode');
...
// banner common shortcode

function banner_common_shortcode()
{
	ob_start();
	get_template_part('template-parts/banner', 'common');
	return ob_get_clean();
}
add_shortcode('banner_common', 'banner_common_shortcode');


// banner common shortcode

function service_services_loop_shortcode()
{
	ob_start();
	get_template_part('template-parts/service', 'services_loop');
	return ob_get_clean();
}
add_shortcode('service_services_loop', 'service_services_loop_shortcode');




// service pricing shortcode

function service_pricing_shortcode()
{
	ob_start();
	get_template_part('template-parts/service', 'pricing');
	return ob_get_clean();
}
add_shortcode('service_pricing', 'service_pricing_shortcode');

// Download Section shortcode

function download_section_shortcode()
{
	ob_start();
	get_template_part('template-parts/download', 'section');
	return ob_get_clean();
}
add_shortcode('download_section', 'download_section_shortcode');


// Download Section shortcode

function map_section_shortcode()
{
	ob_start();
	get_template_part('template-parts/map');
	return ob_get_clean();
}
add_shortcode('map_section', 'map_section_shortcode');



// Contact Section shortcode

function contact_home_shortcode()
{
	ob_start();
	get_template_part('template-parts/contact', 'home');
	return ob_get_clean();
}
add_shortcode('contact_home', 'contact_home_shortcode');


// Contact Section shortcode

function footer_newsleter_shortcode()
{
	return newsletter_form(1);
}
add_shortcode('footer_newsleter', 'footer_newsleter_shortcode');
...
function register_fields()
{
	...
	register_setting('general', 'timing', 'esc_attr');
	add_settings_field('timing', '<label for="timing">'.__('Timing' , 'timing' ).'</label>' , 'print_timing', 'general');
}
...
{
	$value = get_option( 'timing', '' );
	echo '<textarea id="timing" row="4" name="timing" class="regular-text" >' . $value . '</textarea>';
}
